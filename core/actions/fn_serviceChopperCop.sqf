...
_search = nearestObjects[getPos air_cop, ["Air"],10];
...
if (!alive (_search select 0) || (_search select 0) distance air_cop > 15) exitWith {life_action_inUse = false; hint localize "STR_Service_Chopper_Missing"};
...
(_search select 0) setVehicleAmmo 0;
