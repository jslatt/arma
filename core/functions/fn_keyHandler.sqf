...
switch (_code) do {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","C_Hatchback_01_sport_F"]) then {
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                ...
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    if (_locked isEqualTo 2) then {
                        ...
                        [_veh,"car_lock"] remoteExec ["life_fnc_say3D",RANY];
                    };
                };
            };
        };
